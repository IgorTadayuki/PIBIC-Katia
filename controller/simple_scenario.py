            ...
            if dst in TOPOLOGY[src]:
                if dpid in TOPOLOGY[src][dst]:
                    if in_port == TOPOLOGY[src][dst][dpid]['in_port']:
                        ...
                        # sends the packet now, and the next one will be forwarded normally by the switch
                        # use this to avoid losing the first packet before the rule
                        data = None
                        if msg.buffer_id == ofproto.OFP_NO_BUFFER:
                            data = msg.data
